<template>
    <div v-if="showPopupPickup" class="fixed inset-0 bg-black-opacity-45 backdrop-blur-sm flex justify-center items-center z-30" >
        ...
        <div class=" fixed w-screen h-screen items-center justify-center right-[-2rem] bottom-4 flex" >
          ...
          <div class="h-[40rem] w-1/2 p-12 bg-bgColor shadow-2xl rounded-[20px] flex flex-col justify-between items-center font-mont">
            <h1 class="font-bold text-3xl text-secondary text-start mb-[3rem]">
                  ...
                  <form @submit.prevent="pickup" action="" method="post" class="flex flex-col">
                    ...
                    <div class="flex h-[3.125rem] w-[29.5rem] items-center justify-between mb-[1.25rem]">
                      <button
                        type="button"
                        class="relative flex items-center px-4 h-[3.125rem] w-1/2 bg-[#F1F0F0] justify-between rounded-[0.9375rem] focus:outline-none shadow text-gray-600 focus:ring ring-gray-200 group"
                        @click="toggleMenu"
                      >
                      ...
                      <input type="text" v-model="dataPickup.detail_location" name="detail_location" id="detail_location" placeholder="Detail Lokasi" class="bg-[#F1F0F0] h-[3.125rem] px-[1rem] rounded-[0.9375rem] focus:outline-none focus:border-[#31936D]">
                    </div>
                      <div class="flex h-[3.125rem] w-[29.5rem] items-center justify-between mb-[1.25rem]">
                      ...
                      <div class="flex h-[3.125rem] w-[29.5rem] my-[1.75rem] items-center justify-between ">
                          ...
                          <button @click="showAlert " type="submit" class="font-mont font-semibold h-[2.75rem] w-[13.5rem] text-center py-[0.625rem] bg-primary font-[w600] text-white text-xl rounded-[0.9375rem]">
                          Submit
                          </button>
                      </div>
                  </form>

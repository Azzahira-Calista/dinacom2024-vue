<template>
    <div v-if="showPopupPickup" class="fixed inset-0 bg-black-opacity-45 backdrop-blur-sm flex justify-center items-center z-30" >
        ...
        <div class=" fixed w-screen h-screen items-center justify-center right-[-2rem] bottom-4 flex" >
          ...
          <div class="h-[40rem] w-1/2 p-12 bg-bgColor shadow-2xl rounded-[20px] flex flex-col justify-between">
                  ...
                  <form @submit.prevent="pickup" action="" method="post" class="flex flex-col">
                    <!-- mb-[1.875rem] border border-black-->
                      ...
                      <div class="flex h-[3.125rem] w-[29.5rem] my-[1.75rem] items-center justify-between ">
                          <router-link @click="closePopupPickup" to="/" class="font-mont font-semibold h-[2.75rem] w-[7.5rem] text-center py-[0.625rem] bg-gray-400 font-montserrat font-[w600] text-white text-xl rounded-[0.9375rem]">
                              Close
                          </router-link>
  
                          <button @click="showAlert " type="submit" class="font-mont font-semibold h-[2.75rem] w-[13.5rem] text-center py-[0.625rem] bg-primary font-montserrat font-[w600] text-white text-xl rounded-[0.9375rem]">
                          Submit
                          </button>
                      </div>
                  </form>
